...
class RutValidatorUtil {
  String getCheckDigit(String rut) {
    String reversedRUT = rut.split('').reversed.join('');
    int result = 0;
    for (int i = 0, j = 2; i < reversedRUT.length; i++, j < 7 ? j++ : j = 2) {
      result += int.parse(reversedRUT[i]) * j;
    }
    return (11 - (result % 11)) <= 9 ? ((11 - (result % 11))).toString() : 'k';
  }
}
...
class RUTValidator {
  ...
  String? validator(String value) {
    ...
    try {
      ...
      debugPrint('RUT Validation Error: $e');
